    ...
    <script>
      $(function () {
        ...
        $(".fileDrop").on("drop", function (evt) {
          ...
          for (let f of files) {
            // ajax  한번에 하나의 파일씩 업로드 하도록 되어 있기 때문에 반복문을 돌려준다.
      ...
      function displayUploadFile(data) {
        ...
        if (data.image) {
          output +=
            "<img id='" +
            data.originFileName +
            "' class='ufile' src='/resources/upFiles/" +
            data.thumbImgName +
            "' />";
          output +=
            "<img class='delFile' src='/resources/images/delete.png' width='20px' onclick='delFile(this);' />";
        } else {
          output +=
            "<div><a id='" +
            data.originFileName +
            "' href='/resources/upFiles" +
            data.fileNameWithExt +
            "'>" +
            data.originFileName +
            "</a>";
          output +=
            "<img class='delFile' src='/resources/images/delete.png' width='20px' onclick='delFile(this);' /></div>";
        }
        //		if (data.image){
