...
-- 1) substr 이용
select empno, ename as "-이름" , job from emp where substr(ename,2,1) = 'A';
-- 2) instr 이용
select empno, ename as "-이름" , job from emp where instr(ename, 'A') = 2;
-- 3) like 이용
select empno, ename as "-이름" , job from emp where ename like '_A%';
...
select empno, ename, job, sal ,decode(job, upper('analyst'), sal*1.05, sal,
                                      job, upper('salesman'), sal*1.1,  sal,
                                      job, upper('manager'), sal*1.15,  sal,
                                      job ,upper('clerk'), sal*1.2, sal) as upsal from emp;
-- 9. 직업의 종류가 몇개인지 즉, 중복되지 않은 직업의 수를 카운트 하세요
select count(distinct job) from emp;
-- 10. 부서별 사원의 수와 커미션을 받는 사원의 수를 카운트 하세요.
select deptno , count(*) ,count(comm) from emp group by deptno ;
-- count는 null 을 세지 않는다. 0은 숫자로 취급한다.
